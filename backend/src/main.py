...
from .routers import treinos_router

# 2. Cria a instância da aplicação PRIMEIRO
app = FastAPI(
...
# 4. AGORA, registra o manipulador de exceção na 'app' que já existe
...
async def database_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=503,
        content={
            "message": f"Erro de infraestrutura no banco de dados. Verifique se o banco está no ar e se as tabelas foram criadas com 'make db-init'.",
...
# 5. Inclui as rotas do seu projeto
app.include_router(treinos_router.router)


# 6. Define o endpoint raiz
@app.get("/", tags=["Root"])
def read_root():
